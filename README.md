 ...
 * **Recomputing**: Some reactive values (those that implement `Pile`) can recompute the plain value they wrap. 
 ...
 Also, auto-validation must not currently be supressed for that reactive value, as can be done with the `suppressAutoValidation` method.
 ...
 The application programmer defines code that describes how to recompute the value. That code is handed a `Recomputation` object that can be used to interact with the recomputation process, such as:
   ...
   * Manually reconfigure the dependencies after the `Recomputation` has been finished
 ...
 * **Dynamic dependency recording**: Normally, the dependencies of a `Pile` are specified when it is created and remain that way until manually reconfigured.
 ...
 if the dependencies you actually need to access are conditional on the concrete value wrapped by one of other dependencies.
 When used, a `Recomputation` is run in a special *scouting* mode during which it is recorded which reactive values area accessed each time the dependencies may have changed.
 ...
 Almost the only thing the application programmer needs to care about is to avoid doing actual nontrivial computations when the `Recomputation` is in scouting mode, 
 and to access the same dependencies during the actual `Recomputation`.
 * **Transactions**: Let's say we have four reactive values `a`, `r`, `r` and `x` which depend on each other in a diamond pattern:
 ...
 some of its dependencies are invalid due to being in a transaction themselves, enters a transaction that propagates to all its Dependers for as long as the reactive value is temporarily invalid.
 Being in a transaction may invalidate the reactive value, but the plain value it previously held will be remebered; it can be restored and, if a `Recomputation` should be triggered
 ...
 * **Value equivalence**: When a the value wrapped by a reactive value changes, be it by means of setting it manually or because it got recomputed, `ValueEvent`s need to be emitted
 ...
 You can decide by defining a custom equivalence relation that will be used in the place of `Objects::equals`.
 * **Setting with invalid dependencies**: It is possible to manually set a reactive value while some of its dependencies are invalid. This naturally happens
 ...
 can be recomputed from their own dependencies. When the recomputation of such an invalid `Dependency` finishes, its valid `Depender`s should not be invalidated, because they already have
 a good value, and what's worse, they may have been written to manually before they were put to storage, so recomputing would lose these manual changes. However, if an invalid `Dependency`
 ...
 * **Brackets**: A `ValueBracket` represents some kind of effect that should endure for as long as a plain value is held by a reactive value: When the reactive value assumes that value, the bracket
 ...
 * **Associations**: Implementors of the `HasAssociations` offer a simple, typed key-value store that can be used to associate various things with an object. In the *Pile* framework, this is mainly used
 ...
 * **Corrections**: Corrections can be installed on a `CorrigibleValue` to normalize it or veto its change. For example, you my have a reactive value that is meant to hold a unit vector, but
 you would like to be able to write any nonzero vector to it and have it be normalized automatically, and an attempt to write zero vector should be refused.
 * **Sealing**: Implementors of `Sealable` can have their instances optionally be `seal`ed: Attempts to write to them can be ignored, log a warning, throw an exception, or redirect the request.
 ...
 standard utility method `not` to get the inverse of a writable boolean reactive value, then writing to the reactive value that was returned by `not` actually writes the inverse to the orignal reactive boolean. The sealing my be bypassed by parts of the program that have access to a privileged `WriteValue` that must have been obtained before the main value was sealed.
 ...
 * **Builders**: Although you could, you need not configure most aspects of a `Pile` manually. The library comes with a couple of handy builder classes for `Pile`s and `Independent`s that
 allows you to specify behavior using fluent interface syntax and lambdas or method handles.
 * **Specializations**: Wouldn't is be nice if all `ReadDependency<Boolean>` could have a `not` method? Alas, Java does not offer extension methods. So we have specializations of (almost) the whole
 ...
 providing additional methods. But there is also the feature that, for example, you can have a reactive double precision value that recomputes itself as the sum (or product, ...) of all its dependencies that also implement `ReadValueDouble`. This is slightly dangerous though, because it ignores dependencies that are `ReadValueInt` or just `ReadValue<Double>` even though intuitively
 ...
 * **Utilities**: Numerous useful ways are provided to derive a value from one or several others, such as:
   ...
   * `choose`: A reactive value that follows one of several branches, depending on the value of a reactive boolean or the sign of a reactive integer
   ...
   * `field`: A reactive value that follows another reactive value that is extracted from the plain value held by a third rective value
 ...
 * **Remembering the last value**: You may intend some `Independent`s to hold a value that is a setting the user can adjust, and for convenience, when the program is run the next time, the setting should be remembered. But when setting the value (from a file, for example) without user interaction, it should not be remembered. This kind of behaviour is governed by the interfaces `RemembersLastValue`, `LastValueRememberer` and `LastValueRemeberSuppressible`. Utilities are provided that allow you to remember the last value in a `Preferences` node.
 ...
 * **Debugging**: Finding out why one of your `Pile`s recomputes itself when it shouldn't or vice versa can be complicated. To make this and other debugging tasks easier, there are some mechanisms, most of which are disabled by default and can be switched on using the `DebugEnabled` class, allowing you to inspect what's going on with your debgger of choice:
   * Each `Pile` nad `Independent` can have a "name" and an "owner" (or "parent") reference to something it was derived from, or to the larger data structure it belongs to.
   ...
   * The various decisions the lead to the recomputation (or not) of a `Pile` can be documented, to be inspected later in the debbuger. 
   Since this makes the program very slow, it has to be switched on on a per-value basis.
...
The Pile framework has no library dependencies outside the standard library. Within the standard library, it mainly depends on `java.lang(|.ref)` and `java.util(|.function|.concurrent|.logging|.prefs)`. If comes with a small suite of utility classes. There are some dependencies that can be injected; these are concerned with
  ...
  * how to `wait`, `notify`, `sleep`, and `interrupt`. The reasons that I can think of why one might want to do this differently from the native semantics of these methods of `Object` and `Thread` are mainly
