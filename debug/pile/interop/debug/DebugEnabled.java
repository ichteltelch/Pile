...
public class DebugEnabled {
	 ...
	 */
	public static final boolean DE=!DISABLE_ALL_DEBUGGING && false;
	 ...
	 */
	public static final boolean COUNT_BRACKET_LOCKS = !DISABLE_ALL_DEBUGGING && false;
	/**
	 * {@link ValueBracket}s are 
	 * {@linkplain ValueBracket#queued(pile.utils.SequentialQueue) 
	 * normally} opened and closed while the internal mutex of an {@link AbstractReadListenDependency}
	 ...
	 */
	public static final boolean WARN_ON_DESTROY_WHILE_LOCKED = !DISABLE_ALL_DEBUGGING && true;

	/**
	 * Rename threads for the duration of them performing a {@link Recomputation}.
	 */
	public static final boolean RENAME_RECOMPUTATION_THREADS = !DISABLE_ALL_DEBUGGING && !false;
	...
	public static void warn(Logger log, String string) {
		Exception x = new StackTraceWrapper(string, Thread.currentThread().getStackTrace());
		if(log==null)
			x.printStackTrace();
		else
	 ...
	 */
	public static final boolean ERROR_ON_CREATE_IN_DYNAMIC_RECOMPUTATION = 
			!DISABLE_ALL_DEBUGGING && true;

	static final private WeakHashMap<Thread, Object> STOP_REQUESTED = new WeakHashMap<>();
	public static final boolean DEPTH_WARNING = !DISABLE_ALL_DEBUGGING && false;
