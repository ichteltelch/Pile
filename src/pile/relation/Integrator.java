...
public class Integrator<T> {
	...
	private final ValueListener listener = ValueListener.rateLimited(10, 100, this::sourceChanged);



	public final 
	Independent<ReadDependency<? extends Set<? extends ReadWriteListenValue<T>>>>
	sourcesIndir=Piles.<ReadDependency<? extends Set<? extends ReadWriteListenValue<T>>>>independent(Integrator.<ReadWriteListenValue<T>>constEmptySet())
	...
	.bracket(ValueBracket.<Set<? extends ReadListenValue<T>>>make(false, set->{
		for(ListenValue l: set)
			l.addWeakValueListener(listener);
		listener.valueChanged(null);
	}, set->{
		for(ListenValue l: set)
			l.removeWeakValueListener(listener);
	...
	public Integrator(ReadWriteListenDependency<T> target, Supplier<? extends T> neutral, BiFunction<? super T,? super T, ? extends T> integrate) {
		this.target = target;
		this.neutral = neutral;
		this.integrate = integrate;
		monotonous = sources.mapToBool(s->s.contains(target)).validBuffer();
		target.addValueListener_(e->{
			if(target.isValid()) {
				try {
